...
// router.get('/v2/child-details/name-date', (req, res) => {
...
router.get('/v2/informant/informant-triage/05-check', (req, res) => {
  ...
  res.render('/v2/informant/informant-triage/05-check', { data: req.session.data });
});

 // Triage - who is informant - check answers behaviour
  router.get('/v2/informant/01-who-is-form', (req, res) => {
    res.render('informant-triage/01-who-is-informant', { data: req.session.data });
  });
  router.post('/v2/informant/01-who-is-form', (req, res) => {
  ...
  router.get('/v2/informant/03-married-form', (req, res) => {
    res.render('informant-triage/03-married', { data: req.session.data });
  });
  router.post('/v2/informant/03-married-form', (req, res) => {
    req.session.data = req.session.data || {};
    req.session.data.whoIsInCompleted = true;
   // res.redirect('../task-list.html');
     if (req.session.data.checkAnswersTriageVisited) {
  ...
  // // TASK LIST
 router.get('/v2/informant/task-list-inf-form', (req, res) => {
  req.session.data = req.session.data || {};
  ...
  router.get('/v2/informant/prepare-for-appointment-form', (req, res) => {
    res.render('supporting-info/prepare-for-appointment', { data: req.session.data });
  });

  router.post('/v2/informant/prepare-for-appointment-form', (req, res) => {
  ...
  // 1.2: Uploads
  router.get('/v2/informant/uploads-form', (req, res) => {
    res.render('supporting-info/uploads', { data: req.session.data });
  });

  router.post('/v2/informant/uploads-form', (req, res) => {
    req.session.data = req.session.data || {};
    ...
    // 1.2: Comments
  router.get('/v2/informant/comments-form', (req, res) => {
    res.render('supporting-info/comments', { data: req.session.data });
  });

  router.post('/v2/informant/comments-form', (req, res) => {
    req.session.data = req.session.data || {};
   ...
   // 2.0 f35 reason
  router.get('/v2/informant/f35-reason-form', (req, res) => {
    res.render('cause-of-death/f35-reason', { data: req.session.data });
  });

  router.post('/v2/informant/f35-reason-form', (req, res) => {
    req.session.data = req.session.data || {};
  ...
  router.get('/v2/informant/pregnancy-del-form', (req, res) => {
    res.render('cause-of-death/pregnancy-delivery', { data: req.session.data });
  });

  router.post('/v2/informant/pregnancy-del-form', (req, res) => {
    req.session.data = req.session.data || {};
   ...
   // res.redirect('../task-list.html');
    ...
    // 2.2: Cause of death
  router.get('/v2/informant/cause-of-death-form', (req, res) => {
    res.render('cause-of-death/cause-of-death', { data: req.session.data });
  });

  router.post('/v2/informant/cause-of-death-form', (req, res) => {
    req.session.data = req.session.data || {};
 ...
 // 2.2 : Coroner details
  router.get('/v2/informant/coroner-details-form', (req, res) => {
    res.render('cause-of-death/coroner-details', { data: req.session.data });
  });

  router.post('/v2/informant/coroner-details-form', (req, res) => {
    req.session.data = req.session.data || {};
    req.session.data.corDetailsCompleted = true;
   // res.redirect('../task-list.html');
     if (req.session.data.checkAnswersVisited) {
    // If "Check your answers" has been visited, redirect back to it
    res.redirect('/v2/informant/submit/check-answers-page');
  } else {
    // Otherwise, redirect to the next page in the journey (Father's Name page)
    res.redirect('task-list.html');
  }
  ...
  router.get('/v2/informant/medical-info-check-form', (req, res) => {
    res.render('cause-of-death/medical-info-check', { data: req.session.data });
  });

  router.post('/v2/informant/medical-info-check-form', (req, res) => {
    req.session.data = req.session.data || {};
    req.session.data.pregCompleted = true;
    req.session.data.codCompleted = true;
    //res.redirect('../task-list.html');
  ...
  // router.get('/v2/informant/whose-details-form', (req, res) => {
  //   req.session.data = req.session.data || {};
  //   if (req.session.data.pregCompleted && req.session.data.codCompleted) {
  //     res.render('/v2/informant/informant-and-details', { data: req.session.data });
  //   } else {
  //     res.redirect('task-list.html');
  //   }
  // });

  router.post('/v2/informant/whose-details-form', (req, res) => {
    req.session.data = req.session.data || {};
...
// 4.1 child name, sex, date
  router.get('/v2/informant/name-date-form', (req, res) => {
    res.render('child-details/name-date', { data: req.session.data });
  });

  router.post('/v2/informant/name-date-form', (req, res) => {
    req.session.data = req.session.data || {};
  ...
  router.get('/v2/informant/place-of-stillbirth-form', (req, res) => {
    res.render('child-details/place-of-stillbirth', { data: req.session.data });
  });

  router.post('/v2/informant/place-of-stillbirth-form', (req, res) => {
    req.session.data = req.session.data || {};
  ...
  // 5 Parent's details
// 5.1 mothers name
  router.get('/v2/informant/mothers-name-form', (req, res) => {
    res.render('parents-details/mothers-name', { data: req.session.data });
  });

  router.post('/v2/informant/mothers-name-form', (req, res) => {
    req.session.data = req.session.data || {};
  ...
  // 5.2 mothers details
  router.get('/v2/informant/mothers-details-form', (req, res) => {
    res.render('parents-details/mothers-details', { data: req.session.data });
  });

  router.post('/v2/informant/mothers-details-form', (req, res) => {
    req.session.data = req.session.data || {};
  ...
  // 5.3 fathers name
  router.get('/v2/informant/fathers-name-form', (req, res) => {
    res.render('parents-details/fathers-name', { data: req.session.data });
  });

  router.post('/v2/informant/fathers-name-form', (req, res) => {
    req.session.data = req.session.data || {};
  ...
  // 5.4 fathers details
  router.get('/v2/informant/fathers-details-form', (req, res) => {
    res.render('parents-details/fathers-details', { data: req.session.data });
  });

  router.post('/v2/informant/fathers-details-form', (req, res) => {
    req.session.data = req.session.data || {};
...
router.get('/v2/informant/confidential-form', (req, res) => {
    res.render('stats/confidential', { data: req.session.data });
  });

  router.post('/v2/informant/confidential-form', (req, res) => {
    req.session.data = req.session.data || {};
...
// 6.2 Voluntary stats
router.get('/v2/informant/voluntary-form', (req, res) => {
    res.render('stats/voluntary', { data: req.session.data });
  });

  router.post('/v2/informant/voluntary-form', (req, res) => {
    req.session.data = req.session.data || {};
...
// 7.1 Registrars details
  router.get('/v2/informant/register-page-form', (req, res) => {
    res.render('submit/register-page', { data: req.session.data });
  });

  router.post('/v2/informant/register-page-form', (req, res) => {
    req.session.data = req.session.data || {};
  ...
  // 7.2 Check answers
// Route for the Check Your Answers page (GET request)
router.get('/v2/informant/submit/check-answers-page', (req, res) => {
  // Mark the "Check your answers" page as visited
  req.session.data.checkAnswersVisited = true;

  // Render the "Check your answers" page
  res.render('/v2/informant/submit/check-answers-page', { data: req.session.data });
});


  // router.get('/v2/submit/check-answers-form', (req, res) => {
  //   res.render('latest/submit/check-answers-page', { data: req.session.data });
  // });

  // router.post('/v2/submit/check-answers-form', (req, res) => {
  //   req.session.data = req.session.data || {};
